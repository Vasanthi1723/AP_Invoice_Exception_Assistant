...
const API_BASE = "https://ap-invoice-exception-assistant-f4jx.onrender.com";
...
function setupEventListeners() {
  ...
  dom.invoiceUploadZone.addEventListener("click", () =>
    dom.invoiceFileInput.click(),
  );
...
function handleSampleInvoiceClick(filename) {
  ...
  fetch(`${API_BASE}/api/sample-invoices/${filename}`)
    .then((res) => res.blob())
    .then((blob) => {
...
function renderPOPreview(po) {
  dom.poPreviewBody.innerHTML = po.line_items
    .map(
      (item) => `
    <tr>
...
function renderSampleInvoices() {
  ...
  dom.sampleInvoicesContainer.innerHTML = state.sampleInvoices
    .map(
      (inv) => `
...
function renderSummaryStrip(summary, result) {
  ...
  const cards = [
    ...
    {
      cls: "price",
      value: summary.by_type.PRICE_MISMATCH || 0,
      label: "Price Mismatches",
    },
    {
      cls: "qty",
      value: summary.by_type.QUANTITY_MISMATCH || 0,
      label: "Quantity Mismatches",
    },
    {
      cls: "tax",
      value: summary.by_type.TAX_MISMATCH || 0,
      label: "Tax Issues",
    },
    {
      cls: "match",
      value: summary.total_lines_compared - summary.lines_with_exceptions,
      label: "Lines OK",
    },
  ];
...
function renderComparisonTable(result) {
  ...
  result.matched_lines.forEach((ml) => {
    ...
    const priceMatch =
      Math.abs(ml.po_line.unit_price - ml.invoice_line.unit_price) < 0.006;
    const totalMatch =
      Math.abs(ml.po_line.total - ml.invoice_line.total) < 0.01;
    ...
    rows += `<tr>
      ...
      <td class="${qtyMatch ? "cell-match" : "cell-mismatch"}">
        ...
        ${
          !qtyMatch
            ? `<span class="cell-diff cell-diff--${ml.invoice_line.quantity > ml.po_line.quantity ? "over" : "under"}">
      ...
      <td class="${priceMatch ? "cell-match" : "cell-mismatch"}">
        ...
        ${
          !priceMatch
            ? `<span class="cell-diff cell-diff--${ml.invoice_line.unit_price > ml.po_line.unit_price ? "over" : "under"}">
...
function renderExceptions(result) {
  ...
  result.matched_lines.forEach((ml) => {
    ml.exceptions.forEach((exc) => {
      allExceptions.push({
        ...exc,
        context: `Line: ${ml.po_line.description}`,
      });
    });
  });
  ...
  dom.exceptionList.innerHTML = allExceptions
    .map((exc) => {
...
function renderChatSuggestions(result) {
  ...
  const existingTitle = dom.chatSuggestions.querySelector(
...
async function sendChatMessage() {
  ...
  const typingId = addChatMessage(
    "assistant",
    '<span class="loading-step">Thinking…</span>',
  );
  ...
  try {
    ...
    addChatMessage(
      "assistant",
      `<span style="color:var(--severity-high);">Error: ${e.message}</span>`,
    );
  }
}
...
function formatChatResponse(text) {
  ...
  return text
    ...
    .replace(
      /`(.*?)`/g,
      '<code style="background:var(--bg-glass);padding:1px 4px;border-radius:3px;font-family:var(--font-mono);font-size:0.8rem;">$1</code>',
    )
    .replace(/\n/g, "<br>");
}
